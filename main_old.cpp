...
string findAttributesOfEdge(int state, int letter, bool eps);
    ...
    for (int j = 0; j < numLetters; ++j) {
        if (std::find(written.begin(), written.end(), j) == written.end()) {
            ...
            for (int k = 0; k < connections.size(); ++k) {
                ...
                result += "\n            ";
                string letters = allLettersOfEdge(i, connections[k]);
                result += "edge " +
                          findAttributesOfEdge(i, connections[k], letters.find("\\varepsilon") != std::string::npos) +
                          " node {$" +
                          letters + "$} (" +
...
string findAttributesOfEdge(int state, int target, bool eps) {
    if (state == target) {
        if (eps) {
            return "[dotted, loop above]";
        }
        //It is a loop!
        return "[loop above]";
    }
    ...
    if (hasEdgeTo(target, state)) {
        if (eps) return "[dotted, bend left]";
        return "[bend left]";
    }
    if (eps) return "[dotted]";
    return "";
}
